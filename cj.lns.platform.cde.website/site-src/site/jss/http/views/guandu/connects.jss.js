...
exports.doPage = function(frame, circuit, plug, ctx) {
	var doc = ctx.html("/views/guandu/connects.html", "utf-8");
	...
	printConnects(skip,docid,store,doc,portal);
	circuit.content().writeBytes(doc.toString().getBytes());
}
function printConnects(skip,docid,store,doc,portal){
	var ul=doc.select('.connects-msg').first();
	var ali=ul.select('>li').first().clone();
	ul.empty();
	var limit=100000;
	var personCount=0;
	var totalTimes=0;
	var data=store.connectsGroupByUser(docid,limit,skip);
	...
	doc.select('.connects-count span[count]').html(personCount+'&nbsp;人');
	doc.select('.connects-count span[times]').html(totalTimes+'&nbsp;次');
