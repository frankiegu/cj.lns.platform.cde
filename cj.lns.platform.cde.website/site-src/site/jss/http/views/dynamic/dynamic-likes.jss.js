...
exports.doPage = function(frame, circuit, plug, ctx) {
	var doc = ctx.html("/views/dynamic/dynamic-likes.html", "utf-8");
...
function printVisitors(skip,docid,home,doc,portal){
	var ul=doc.select('.likes-msg').first();
	...
	var it=home.aggregate("cde.dynamics.likes", pipeline).iterator();
	...
	doc.select('.likes-count span[count]').html(personCount+'&nbsp;人');
	doc.select('.likes-count span[times]').html(totalTimes+'&nbsp;次');
