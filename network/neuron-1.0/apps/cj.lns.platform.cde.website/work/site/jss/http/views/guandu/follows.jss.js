...
exports.doPage = function(frame, circuit, plug, ctx) {
	var doc = ctx.html("/views/guandu/follows.html", "utf-8");
	...
	printFollows(skip,docid,store,doc,portal,subject);
	circuit.content().writeBytes(doc.toString().getBytes());
}
function printFollows(skip,docid,store,doc,portal,subject){
	var ul=doc.select('.follows-msg').first();
	var ali=ul.select('>li').first().clone();
	ul.empty();
	var limit=10000000;
	var personCount=0;
	var totalTimes=0;
	var data=store.followsGroupByUser(docid,limit,skip);
	var ismyfollow=false;//当前主题是否对它有关注，如果已关注则显示为取消关注，否则为已关注
	var op=doc.select('.follows .op');
	...
	doc.select('.follows-count span[count]').html(personCount+'&nbsp;人');
	doc.select('.follows-count span[times]').html(totalTimes+'&nbsp;次');
